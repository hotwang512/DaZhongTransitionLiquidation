...
namespace DaZhongTransitionLiquidation.Areas.AssetManagement.Controllers.ReviewAsset
{
    public class AssetModifyReviewController : BaseController
    {
        ...
        {
            var jsonResult = new JsonResultModel<Business_ModifyVehicle>();

            DbBusinessDataService.Command(db =>
            {
                int pageCount = 0;
                para.pagenum = para.pagenum + 1;
                jsonResult.Rows = db.Queryable<Business_ModifyVehicle>()
                    .Where(i => !i.ISVERIFY)
        ...
        public JsonResult SubmitModifyVehicleReview(List<Guid> guids,string MODIFY_TYPE)
        {
            ...
            DbBusinessDataService.Command(db =>
            {
                var modifyVehicleList = db.Queryable<Business_ModifyVehicle>().Where(x => guids.Contains(x.VGUID) && x.MODIFY_TYPE == MODIFY_TYPE).ToList();

...
{
    public class ReimburseOrderController : BaseController
    {
        // GET: ReimbursementCenter/ReimburseOrder
        public ReimburseOrderController(DbService dbService, DbBusinessDataService dbBusinessDataService) : base(dbService, dbBusinessDataService)
        {
        ...
        public JsonResult GeReimbursementListDatas(Business_ReimburseOrder searchParams, GridParams para)
        {
            var jsonResult = new JsonResultModel<Business_ReimburseOrder>();

            DbBusinessDataService.Command(db =>
            {
                int pageCount = 0;
                para.pagenum = para.pagenum + 1;
                jsonResult.Rows = db.Queryable<Business_ReimburseOrder>()
        ...
        public JsonResult DeleteReimbursementOrder(List<Guid> vguids)
        {
            ...
            DbBusinessDataService.Command(db =>
            {
                ...
                var isAnySubmited = db.Queryable<Business_ReimburseOrder>().Any(c => vguids.Contains(c.VGUID) && (c.SubmitStatus == FixedAssetsSubmitStatusEnum.Submited.TryToInt() || c.SubmitStatus == FixedAssetsSubmitStatusEnum.UnPay.TryToInt()));
                if (isAnySubmited)
                {
                ...
                {
                    //删除主表信息
                    saveChanges = db.Deleteable<Business_ReimburseOrder>(x => vguids.Contains(x.VGUID)).ExecuteCommand();
        ...
        public JsonResult SubmitReimbursementOrder(List<Guid> vguids)
        {
                ...
                var result = db.Ado.UseTran(() =>
                {
                    foreach (var vguid in vguids)
                    {
                        ...
                        if (model.SubmitStatus == FixedAssetsSubmitStatusEnum.UnSubmit.TryToInt())
                        {
                            ...
                            {
                                var orderModel = db.Queryable<Business_ReimburseOrder>()
                                ...
                                db.Updateable<Business_ReimburseOrder>(orderModel).UpdateColumns(x => new { x.PaymentVoucherUrl, x.PaymentVoucherVguid }).ExecuteCommand();

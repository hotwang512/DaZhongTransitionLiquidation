...
{
    public class PurchaseFaxOrderSettingController : BaseController
    {
        // GET: SystemManagement/PurchaseFaxOrderSetting
        public PurchaseFaxOrderSettingController(DbService dbService, DbBusinessDataService dbBusinessDataService) : base(dbService, dbBusinessDataService)
        {
        ...
        public JsonResult GetPurchaseFaxOrderSettingListDatas(Business_PurchaseFaxOrderSetting searchModel, GridParams para)
        {
            var jsonResult = new JsonResultModel<Business_PurchaseFaxOrderSetting>();
            DbBusinessDataService.Command(db =>
            {
                int pageCount = 0;
                para.pagenum = para.pagenum + 1;
                jsonResult.Rows = db.Queryable<Business_PurchaseFaxOrderSetting>()
        ...
        public JsonResult DeletePurchaseFaxOrderSetting(List<Guid> vguids)//Guid[] vguids
        {
            ...
            DbBusinessDataService.Command(db =>
            {
                int saveChanges = 1;
                //删除主表信息
                saveChanges = db.Deleteable<Business_PurchaseFaxOrderSetting>(x => vguids.Contains(x.VGUID)).ExecuteCommand();

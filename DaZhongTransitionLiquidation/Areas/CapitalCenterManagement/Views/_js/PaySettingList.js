...
var $page = function () {
        ...
        selector.$grid().jqxDataTable(
            {
                ...
                columns: [
                    ...
                    { text: '借', datafield: "Borrow", width: '180px', align: 'center', cellsAlign: 'center', hidden: true },
                    { text: '贷', datafield: "Loan", width: '180px', align: 'center', cellsAlign: 'center', hidden: true },
    ...
    function channelDetailFunc(row, column, value, rowData) {
        var container = "<a href='#' onclick=link('" + rowData.VGUID + "') style=\"text-decoration: underline;color: #333;\">" + rowData.BankAccount + "</a>";
        return container;
        //var container = "";
        //var borrow = "";
...
function link(VGUID) {
    window.location.href = "/CapitalCenterManagement/PaySettingDetail/Index?VGUID=" + VGUID;
}
...
function initBorrowTable(companyCode, accountMode, type, indexs) {
    var source = {
        datafields:
        [
    ...
    $("#grid1").on('rowclick', function (event) {
        ...
        var value = row.BusinessCode;
        if (type == "B") {
            $("#Borrow" + indexs).val(value);
        } else {
            $("#Loan" + indexs).val(value);
        }
    });
}
...
function selectSevenSubjectB(indexs) {
    initBorrowTable(companyCode, accountMode, "B", indexs);
    $("#AddSelectSevenDialog").modal({ backdrop: "static", keyboard: false });
    $("#AddSelectSevenDialog").modal("show");
}
function selectSevenSubjectL(indexs) {
    initBorrowTable(companyCode, accountMode, "L", indexs);
    $("#AddSelectSevenDialog").modal({ backdrop: "static", keyboard: false });
    $("#AddSelectSevenDialog").modal("show");
}
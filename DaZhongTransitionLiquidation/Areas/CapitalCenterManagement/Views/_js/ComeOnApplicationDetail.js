var $page = function () {
    ...
    this.init = function () {
        addEvent();
        getComeOnApplicationDetail();
    };
    var selector = this.selector = {};

    function addEvent() {
        var accountMode = $("#LoginAccountModeCode").val();
        $("#TurnOutAccountModeCode").val(accountMode);
        $("#TurnInAccountModeCode").val(accountMode)
        ...
        $("#Preview").on("click", function () {
            ...
            if (turnInCompanyName == turnOutCompanyName) {
                jqxNotification("调出公司与调入公司不能相同！", null, "error");
                return;
            }
        ...
        $("#btnSave").on("click", function () {
            $.ajax({
                url: "/CapitalCenterManagement/ComeOnApplicationDetail/SaveComeOnApplicationDetail",
                data: {
...
function getComeOnApplicationDetail() {
    var guid = $.request.queryString().VGUID;
    $.ajax({
        url: "/CapitalCenterManagement/ComeOnApplicationDetail/GetComeOnApplication",
        data: {
            "vguid": guid,
        },
        type: "post",
        ...
        success: function (msg) {
            ...
            if (msg.Money != "" && msg.Money != null) {
                $("#TurnOut").show();
                $("#Money").val(msg.Money);
                var value = smalltoBIG(msg.Money); $("#MoneyA").val(value);
            } else {
                $("#TurnIn").show();
                $("#TurnOut").hide();
                $("#TurnInMoney").val(msg.TurnInMoney);
                var value = smalltoBIG(msg.TurnInMoney); $("#TurnInMoneyA").val(value);
            }
        }
    });
    if ($('#Status').val() == "") {

        ...
        public JsonResult SaveCompanySection(Business_SevenSection sevenSection, bool isEdit)
        {
            ...
            DbBusinessDataService.Command(db =>
            {
                ...
                {                  
                    ...
                    var isAny = db.Queryable<Business_SevenSection>().Any(x => x.Code == code && x.AccountModeCode == sevenSection.AccountModeCode && x.CompanyCode == sevenSection.CompanyCode && x.VGUID != guid && x.SectionVGUID == "D63BD715-C27D-4C47-AB66-550309794D43");
                    if (isAny)
                    {
                        IsSuccess = "2";
                        return;
